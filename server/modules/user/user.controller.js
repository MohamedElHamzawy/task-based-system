...
const showAllUsers = async (req,res,next) => {
    try {
        const allUsers = await userModel.find({}).populate(["speciality", "country"]);
...
const getCustomerService = async (req,res,next) => {
    try {
        const allUsers = await userModel.find({user_role: "customerService"}).populate(["speciality", "country"]);
        res.json({users: allUsers});
    } catch (error) {
        return next(new HttpError(`Unexpected Error: ${error}`, 500));
    }
...
const getSpecialistService = async (req,res,next) => {
    try {
        const allUsers = await userModel.find({user_role: "specialistService"}).populate(["speciality", "country"]);
        res.json({users: allUsers});
    } catch (error) {
        return next(new HttpError(`Unexpected Error: ${error}`, 500));
    }
...
const filterSortedUsers = async (req,res,next) => {
    try {
        ...
        if (sort && role) {
            if (sort == "completed") {
                const allUsers = await userModel.find({user_role: role}).populate(["speciality", "country"]).sort({completedCount: -1});
                res.json({users: allUsers});
            } else if (sort == "profit") {
                const allUsers = await userModel.find({user_role: role}).populate(["speciality", "country"]).sort({totalProfit: -1});
                res.json({users: allUsers});
            }         
        } else if (sort) {
            if (sort == "completed") {
                const allUsers = await userModel.find({}).populate(["speciality", "country"]).sort({completedCount: -1});
                res.json({users: allUsers});
            } else if (sort == "profit") {
                const allUsers = await userModel.find({}).populate(["speciality", "country"]).sort({totalProfit: -1});
                res.json({users: allUsers});
            }     
        } else if (role) {
            const allUsers = await userModel.find({user_role: role}).populate(["speciality", "country"]);
            res.json({users: allUsers});
        } else {
