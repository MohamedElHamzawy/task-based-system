...
const refuseTask = async (req, res, next) => {
  try {
    ...
    await taskModel.findOneAndUpdate(
      { _id: taskID },
      { taskStatus: statusID._id, cost: 0 },
      { new: true }
    );
    await noteModel.create({
      task_id: taskID,
      content: `Offer Refused by ${req.user.full_name}`,
...
const acceptTask = async (req, res, next) => {
  try {
    ...
    await taskModel.findOneAndUpdate(
      { _id: taskID },
      { paid: paid, taskStatus: statusID._id },
      { new: true }
    );
    await noteModel.create({
      task_id: taskID,
      content: `Offer Approved by ${req.user.full_name}`,
      user_id: req.user._id,
    });
